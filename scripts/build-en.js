...
const nightlyRates = Object.values(JSON.parse(fs.readFileSync(path.join(ROOT, 'prezzi.json'), 'utf8')));
assert.ok(nightlyRates.length === 12 && nightlyRates.every(rate => Number.isFinite(rate) && rate > 0), '12 valid monthly rates');
const minNightlyRate = Math.min(...nightlyRates);
const maxNightlyRate = Math.max(...nightlyRates);
...
function build(page, lang) {
    let html = read(`${page}.html`).replace('<!-- COOKIE_NOTICE -->', read('cookie-notice.html'))
        .replaceAll('{{MIN_NIGHTLY_RATE}}', String(minNightlyRate))
        .replaceAll('{{MAX_NIGHTLY_RATE}}', String(maxNightlyRate));
    ...
    if (page === 'index') {
        ...
        html = html.replace(/<script type="application\/ld\+json">([\s\S]*?)<\/script>/g, (_, json) => {
            ...
            if (data['@type'] === 'VacationRental') {
                // One property identity across both language versions.
                data.url = `${ORIGIN}/`;
                data.mainEntityOfPage = {
                    '@type':'WebPage', '@id':urlFor(page,lang)+'#webpage',
                    url:urlFor(page,lang), name:title, inLanguage:lang,
                    isPartOf:{'@id':`${ORIGIN}/#website`},
                    mainEntity:{'@id':`${ORIGIN}/#apartment`},
                };
                if (lang === 'en') {
            ...
            if (data['@type'] === 'FAQPage') {
                data['@id']=urlFor(page,lang)+'#faq';
                data.url=data['@id'];
                data.inLanguage=lang;
                data.about={'@id':`${ORIGIN}/#apartment`};
                data.isPartOf={'@id':urlFor(page,lang)+'#webpage'};
                data.mainEntity=faq;
            }
