...
// Inline the complete home cascade: one fewer render-blocking round trip, with
// no asynchronous style loader or unstyled calendar/content when JS is absent.
...
// A stylesheet URL is relative to the CSS file; inline CSS is relative to the
// page. Absolute asset paths keep fonts and the hero correct on /en/ and /it/.
const inlineHomeCss = homeCss.replace(/url\((['"]?)(?=(?:img|vendor)\/)/g, 'url($1/');
const meta = {
    index: {
...
function build(page, lang) {
    ...
    if (page === 'index') {
        assert.ok(html.includes('<!-- HOME_STYLES -->'), 'home stylesheet insertion point');
        html = html.replace('<!-- HOME_STYLES -->', `<style id="home-styles">\n${inlineHomeCss}\n    </style>`);
...
// Compatibility for previously served/cached HTML using /home.css?v=29.
// Current home pages embed this cascade and never request the separate asset.
